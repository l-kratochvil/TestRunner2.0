...
- Testy se z assembly nenačítají: https://claude.ai/share/2111234d-351e-4c9b-93c9-846f62da3015
- VIZ PAPÍR
- Implementovat MVVM a udělat služby z testlink api a runneru:
  - viz DEV-NOTES.md
  - ITestLinkApi (Service)
  - IConfigFile (Model); IFileSystem (server) : IFileSystem.ReadConfigXml(path) (metoda návrátí Model.IConfigXml)
  - ITestRunnerEngine (Service)
- Zautomatizovat kapitolu 2.9.3 v dokumentu Automatizované testováno
- Update nuget balíčků
- Localize texty
...
- Layout:
  - Klasicky runner
  - vlevo průzkumník
  - vpravo: levá část konfigurace (Runtime, atd.), pravá část výsledky testů?
  - dole logger, nahoře main menu ...
- Grill-me session: BaseStore,
- server by běžel na virtuálu (vmware) - stroj by se nikdy nesměl zamknout a test by mohl běžet kdokoliv. Server by řešil spuštění testu (skrze rpc)
- Client:
  - Prohlížeč by umožnoval, ze by test mohl být spuštěn odkud koliv a kdykoliv by bylo možné zkontrolovat průběh testu
  - Při dokončení by uživatel dostal notifikaci
  - Bylo by třeba řídit přístup (autorizace)
- Pokud by se vyřešilo provádění instalací, tak by se mohlo využívat maximálně vzdáleně
- K reportum by se do TL nahrávaly scshoty
- AUTH:
