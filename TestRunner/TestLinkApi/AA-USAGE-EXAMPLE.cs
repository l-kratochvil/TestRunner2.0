...
using TestRunner.TestLinkApi.Types;

// TODO: REFACTOR!

namespace TestRunner.TestLinkApi
{
    class Program
    {
        ...
        private readonly ITestLinkApiClient apiClient = App.Services.GetService<ITestLinkApiClient>() ??
        ...
        {
            var suite = new TestSuite(testSuite._id, testSuite._name, testSuite._details, testSuite._nodeOrder, testSuite._nodeTypeId,
                testSuite._parentId);
            ...
            (TestStatus status, int testPlanId, int testSuiteId, int testCaseId, string notes)[] Result)
        {
            foreach (var result in Result)
            {
                ...
                TestCaseFromTestSuite[] testsuiteTestcases = apiClient.GetTestCasesForTestSuite(result.testSuiteId, true);
