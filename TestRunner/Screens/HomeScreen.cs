...
internal sealed class HomeScreen : BaseScreen
{
    protected override ScreenRenderer CreateRenderer() => new()
    {
        ...
        Info = () =>
        {
            var testSuites = TestRunConfig.Current.TestEntities.Where(e => e.Type == ITestEntity.TypeKind.TestSuite).ToArray();
            var testCases = TestRunConfig.Current.TestEntities.Where(e => e.Type == ITestEntity.TypeKind.TestCase).ToArray();
