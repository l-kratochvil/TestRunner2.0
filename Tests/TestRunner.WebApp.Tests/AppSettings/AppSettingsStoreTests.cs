...
public class AppSettingsStoreTests
{
    ...
    public void SetUp()
    {
        this.filePath = Path.Combine(
            Path.GetTempPath(),
            $"testrunner-settings-{Guid.NewGuid():N}",
            "user-settings.json");

        this.unit = new AppSettingsStore(this.CreateStorage());
    }
    ...
    public async Task StartAsync__WhenTheSettingsWereSavedBefore__ThenShouldPutThemBack()
    {
        ...
        AppSettingsStore restarted = new(this.CreateStorage());
    ...
    private JsonFileStorage<AppSettingsState> CreateStorage()
        => new(
            this.filePath,
